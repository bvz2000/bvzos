...
class ScanFiles(object):
    ...
    def _append_to_scan(self,
        ...
        self.files[file_path] = file_metadata
    ...
    def scan_files(self,
        ...
        for file_p in files_p:
            ...
            self.scan_file(file_p=file_p, root_p=root_p, uid=self.options.uid, gid=self.options.gid)
            if self.checked_count % self.options.report_frequency == 0:
                yield self.checked_count
    ...
    def append_file(self,
                    ...
                    root_p):
        """
        Appends a single file to a scan that has already been run.

        :param file_p:
            The path to the file to be appended to the scan.
        :param root_p:
            The root path from which to calculate the relative path.
    ...
    def scan_file(self,
                  ...
                  gid):
        """
        Scan a single file and store its metadata.
        ...
        if self.options.skip_hidden_files:
            if bvzos.fs.path.is_hidden(file_p=file_p):
        ...
        try:
            attrs = metadata.get_metadata(file_p=file_p, root_p=root_p)
        except FileNotFoundError:
            self.error_count += 1
        ...
        if not bvzos.fs.path.has_file_read_permissions(st_mode=attrs["st_mode"],
        ...
        self._append_to_scan(file_path=file_p, file_metadata=attrs)
